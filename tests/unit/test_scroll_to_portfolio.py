...
def test_scroll_to_portfolio_delay_constant():
    ...
    # Extract the value (simple regex for the assignment)
    import re
...
def test_scroll_to_portfolio_session_state_key_in_defaults():
    ...
    # Verify the key exists in the session state defaults
    assert '"scroll_to_portfolio"' in content or "'scroll_to_portfolio'" in content

    # Verify it's initialized to False
    assert (
        '"scroll_to_portfolio": (\n            False,' in content
        or "'scroll_to_portfolio': (\n            False," in content
    )
