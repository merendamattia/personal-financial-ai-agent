...
def _test_ollama_connection(base_url: str, model: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if parsed.hostname not in allowed_hosts:
            return (
                False,
                "❌ Only localhost or docker 'ollama' hostname are allowed for security.",
            )
        ...
        verify_ssl = not health_url.startswith(
            "http://localhost"
        ) and not health_url.startswith("http://127.0.0.1")
...
def show_settings_page():
    ...
    with st.expander("✨ **OpenAI Configuration**", expanded=True):
        ...
        with col1:
            ...
            openai_model = st.text_input(
                "Model",
                value=st.session_state.settings_config.get(
            ...
            if st.button("🔌 Test OpenAI", use_container_width=True, key="test_openai"):
                if not openai_api_key:
                    ...
                    with st.spinner("Testing connection..."):
                        success, message = _test_openai_connection(
        ...
        if st.button(
            "💾 Save OpenAI Settings", use_container_width=True, key="save_openai"
        ):
    ...
    with st.expander("🌐 **Google Gemini Configuration**", expanded=False):
        ...
        with col2:
            ...
            if st.button("🔌 Test Google", use_container_width=True, key="test_google"):
                if not google_api_key:
                    ...
                    with st.spinner("Testing connection..."):
                        success, message = _test_google_connection(
        ...
        if st.button(
            "💾 Save Google Settings", use_container_width=True, key="save_google"
        ):
    ...
    with st.expander("🦙 **Ollama Configuration**", expanded=False):
        ...
        if st.button(
            "💾 Save Ollama Settings", use_container_width=True, key="save_ollama"
        ):
