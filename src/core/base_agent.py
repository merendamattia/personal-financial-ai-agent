"""
Base Financial Agent Module.

This module defines the BaseAgent abstract class that encapsulates
common configuration and initialization logic for all specialized financial agents.
"""
...
class BaseAgent(ABC):
    """
    Abstract base class for specialized Financial AI Agents.

    Encapsulates common configuration, initialization, and utility methods
    that all specialized agents share. Subclasses must implement _get_default_tools().
    """
    ...
    def __init__(
        ...
        Args:
            ...
            planning_interval: Planning interval for agent (default from subclass)
            max_steps: Maximum steps for agent (default from subclass)
        """
        logger.debug(
            "Initializing %s with provider=%s", self.__class__.__name__, provider
        )

        # Load configuration from environment
        self.name = name or os.getenv("AGENT_NAME", f"{self.__class__.__name__}")
        ...
        # Store or get default planning interval and max steps
        ...
        logger.debug(
            "Agent parameters: planning_interval=%d, max_steps=%d",
            self.planning_interval,
            self.max_steps,
        )
        ...
        # Load or use provided system prompt
        if system_prompt:
    ...
    def _get_default_system_prompt(self) -> str:
        ...
        Subclasses must implement this to provide their specialized prompts.
    ...
    def _get_default_tools(self) -> list:
        ...
        Subclasses must override this to provide their specific tools.
    ...
    def _get_default_planning_interval(self) -> int:
        """
        Get the default planning interval for this agent.

        Subclasses must override this to provide their specific planning interval.

        Returns:
            Planning interval as integer
        """
        pass
    ...
    def _get_default_max_steps(self) -> int:
        """
        Get the default max steps for this agent.

        Subclasses must override this to provide their specific max steps.

        Returns:
            Max steps as integer
        """
        pass
